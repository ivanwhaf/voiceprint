...
from dataset import AISHELL1, AISHELL1Online
from loss import *
from model import ECAPA_TDNN

parser = argparse.ArgumentParser()
parser.add_argument('-project_name', type=str, help='project name', default='voiceprint_aishell1_ecapatdnn_online')
...
parser.add_argument('-dataset_path', type=str, help='path of dataset',
                    default='H:/Data/datasets/Voiceprint/myairbridge-AISHELL-1/data_aishell/npy')
...
def create_online_dataset():
    train_set = AISHELL1Online("train")
    val_set = AISHELL1Online("val")
    test_set = AISHELL1Online("test")
...
if __name__ == "__main__":
    ...
    train_set, val_set, test_set = create_online_dataset()
    ...
    num_per_class = 300 * 30
    num_per_epoch = int(num_per_class / args.epochs)

    for epoch in range(args.epochs):
        # indices = np.zeros(int(300 / 100) * args.num_classes)
        indices = []
        for n in range(args.num_classes):
            start_idx = n * num_per_class
            indices.extend(
                [i for i in range(start_idx + epoch * num_per_epoch, start_idx + (epoch + 1) * num_per_epoch)])

        sub_set = Subset(train_set, indices)
        train_loader = DataLoader(sub_set, batch_size=args.batch_size, shuffle=True, num_workers=1)
